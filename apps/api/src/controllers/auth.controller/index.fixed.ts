...
export async function registerMember(
  ...
  try {
    ...
    // Instead of calling sendEmailVerificationCode directly, prepare for it
    // and let the next middleware handle it
    next();
  } catch (error) {
    next(error);
  }
...
export async function loginMember(
  ...
  try {
    ...
    // Instead of calling sendEmailVerificationCode and then sending another response,
    // we'll handle the email sending here
    const verificationCode = (await generateEmailVerificationCode(email, type)).code;
    ...
    let verifyEmailTemplateCompiled = compile(verifyEmailTemplate)({
      email: email,
...
export async function sendEmailVerificationCode(
  ...
  try {
    ...
    // Only send a response if this is a direct API call
